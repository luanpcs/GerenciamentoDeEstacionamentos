function registraLogin(user, senha) {
...
function buscarRegistroTotal() {
    const url = 'http://localhost:3000/registrosTotais';
    ...
    return fetch(url)
        ...
        .then(data => {
            const cadastros = data.map(item => ({
                ...
                modelo: item.modelo,
                placa: item.placa,
                timestampEntrada: item.timestampEntrada,
                timestampSaida: item.timestampSaida,
                vaga: item.vaga,
                valor: item.valor
            }));
            return cadastros;
...
function enviarRegistroTotal(nome, modelo, placa, timestampEntrada, timestampSaida, vaga, valor) {
    const url = 'http://localhost:3000/registrosTotais';
